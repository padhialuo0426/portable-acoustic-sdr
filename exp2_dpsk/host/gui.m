...
%   两边都要改**。
    ...
    A.fs = 8000; A.fm = 100; A.fc = 1000;
    A.N = A.fs/A.fm;                 % 每码元样本数 = 80
    A.T = 1/A.fm;                    % 码元时间
    A.beta = 0.5;                    % 成形滤波滚降系数
    A.mseq  = [1 0 0 1 1 0 1 0 1 1 1 1 0 0 0];
    A.GUARD = 80;                    % 帧尾保护码元：盖过模型 40 码元的流水延迟

    %% ------------------------- 界面骨架 -------------------------
    fig = uifigure('Name','实验二 · DPSK —— 一键声学实测', ...
    ...
    A.btnDec   = mkButton(gL,'仅解码已取回的 dpsk5.mat',@onDecodeOnly);
    ...
    function onCheck(~,~)
        ...
        try
            ...
            [st,~] = ssh(sprintf('test -x %s/build/dpsk_rx', remoteDir()));
            if st == 0
                logStep('板上可执行', '✓', '已就绪');
            else
    ...
    function onRun(~,~)
        ...
        try
            ...
            bgssh(sprintf('cd %s && rm -f dpsk5.mat && ./build/dpsk_rx -d %s -t %d', ...
            ...
            % 4) 取回
            localMat = fullfile(A.here,'dpsk5.mat');
            [st,out] = scpFrom(sprintf('%s/dpsk5.mat', A.rdir.Value), localMat);
            if st ~= 0
                logStep('取回数据', '✗', '%s', firstLine(out));
                logf('  板上可能没产出 dpsk5.mat（采集设备打不开？见 Q&A Q1/Q2）');
                return
            end
            logStep('取回数据', '✓', 'dpsk5.mat');
    ...
    function onDecodeOnly(~,~)
        ...
        try
            ...
            doDecode(fullfile(A.here,'dpsk5.mat'), info_all, NN, MM);
        catch e
    ...
    function doDecode(matfile, info_all, NN, MM)
        if ~isfile(matfile)
            logStep('解码', '✗', '本地没有 dpsk5.mat，先做一次实测'); return
        end
        S = load(matfile);
    ...
    function [x, info_all, code, NN, MM] = buildWaveform()
        ...
        info(27:41)       = A.mseq;                     % 帧头
        info(42:41+L)     = info_all;                   % 图片信息
        info(42+L:56+L)   = A.mseq;                     % 帧尾
        ...
        den = 1 - 16*b*b*z.*z;
        h1  = (4*b/(pi*sqrt(A.T))) * num ./ den;

        % conv(upsample(ds,N), h1) 后调制到载波
        sq = conv(upsample(ds, A.N), h1);
        idx = 0:numel(sq)-1;
        x = sq .* sin(2*pi*A.fc*idx/A.fs);
        x = x / max(abs(x));
    end
    ...
    function [ber, bmp, l1, l2, flag] = decodeStream(xs, info_all, NN, MM)
        ...
        for fl = [1 -1]
            loc = [];
            for p = 1:numel(xs)-14
                if xs(p:p+14)*fl*pm > sum(abs(xs(p:p+11))) && abs(xs(p)) > thr
    ...
    function refreshTiming()
        try
            ...
            code = 56 + L + A.GUARD;
            dur  = (code * A.N + 2*A.N - 1) / A.fs;   % 含成形滤波器拖尾
            A.durTxt.Text = sprintf('%.1f s（%d 码元 / %d 位）', dur, code, L);
