function [x,sym]=v22_modulate(bits,P,backend)
%V22_MODULATE  V.22bis 音频；backend 默认 matlab，可选 simulink。
here=fileparts(fileparts(mfilename('fullpath')));addpath(fullfile(here,'lib'));
...
if strcmp(backend,'matlab')
 ...
 sdrTxRequest=struct('frameBits',bits,'P',P); %#ok<NASGU>
 run(fullfile(fileparts(fileparts(mfilename('fullpath'))),'v22_emit.m'));
 return
end
