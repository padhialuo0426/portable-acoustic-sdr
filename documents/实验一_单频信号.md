...
3. 配置：`System target file = ert.tlc`，`Hardware board = None`，`Language = C`，`GenCodeOnly = on`，`MatFileLogging = off`（避免 `To File` 拖入 `rt_logging`，见 [Q&A](Q&A.md)），`Device Type = ARM Cortex-A (64-bit)`，`Toolchain = Automatically locate an installed toolchain`（见 [Q10](Q&A.md)）。
