...
├── src/              板级运行时（手写，可移植）
├── include/          运行时↔模型 契约
├── simulink_model/   改造后的 Simulink 模型 + 生成的纯算法 C 代码
└── host/             PC 端脚本：发射(single_fre_emit) + 频谱分析(spectrum)
```
...
3. 配置：`System target file = ert.tlc`，`Hardware board = None`，`Language = C`，`GenCodeOnly = on`，`MatFileLogging = off`（避免 `To File` 拖入 `rt_logging`，见 [Q&A](Q&A.md)），`Device Type = ARM Cortex-A (64-bit)`，`Toolchain = Automatically locate an installed toolchain`（见 [Q11](Q&A.md)）。
4. `Sum left & right channels and to single/Matrix Sum` 勾选 **Saturate on integer overflow**：该块输出 `int16`，单声道采集时左右同源 ⇒ 求和为 `2x`，不饱和会回绕翻转（见 [Q5](Q&A.md)）。
...
```matlab
cd <exp1_single_freq/simulink_model>
