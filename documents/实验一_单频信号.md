...
├── board/            板上的一切
│   ├── single_fre_rev.slx        Simulink 模型（只在 PC 上打开）
...
| 文件 | 作用 |
|---|---|
| `single_fre_rev.slx` | 接收模型：两路带通滤波 + 记录。**它只在 PC 上用 Simulink 打开、生成下面那份 C**，不会被同步到板上。 |
...
cd(fullfile(R,'exp1_single_freq','board'));   % 模型和生成目录都在这儿
...
1. 打开 `single_fre_rev.slx`（就在当前文件夹里）。
