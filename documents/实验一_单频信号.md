...
│   └── single_fre_rev_ert_rtw/   Simulink 生成的纯算法 C
...
### `board/single_fre_rev_ert_rtw/` — Simulink 生成的 C

| 文件 | 作用 |
|---|---|
| `board/single_fre_rev_ert_rtw/*.c/.h` | Simulink 生成的**纯算法 C 代码**（无任何支持包/rt_logging 依赖）。Makefile 直接读取该目录，无需手动拷贝。 |
...
| 文件 | 作用 |
|---|---|
| `single_fre_rev.slx` | 接收模型：两路带通滤波 + 记录。 生成代码落在 `../board/`，见下面「重新生成模型」。 |
...
两种等价方式，产物都落到 `board/single_fre_rev_ert_rtw/`，任选其一。
...
cd(fullfile(R,'exp1_single_freq','board'));
addpath(fullfile(R,'exp1_single_freq','host'));     % .slx 在 host/，加进路径才找得到
...
slbuild('single_fre_rev');     % 生成到 board/single_fre_rev_ert_rtw/
...
> ⚠️ **动手前先把 MATLAB 的当前文件夹切到 `exp1_single_freq/board/`**（左侧地址栏，或命令行 `cd`）。
> 代码生成到哪，由**当前文件夹**决定——Simulink 的「代码生成文件夹」默认就是它。
> 不切的话代码会落到你当时所在的目录（比如 `exp1_single_freq/` 根下），`make` 编的还是
> `board/` 里的旧代码。

GUI 方式就是把上面 `set_param` + `slbuild` 用菜单点出来，产物完全一致。

1. 打开 `host/single_fre_rev.slx`。
