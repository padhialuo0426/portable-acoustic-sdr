...
滤波、记录，PC 端做 FFT 验证频谱峰值落在 1kHz。用来打通"发射→声学信道→板端
接收→落盘→PC 分析"的整条最小链路。

## 数据流

```
[PC] host/single_fre_emit.m  --sound()-->  扬声器 ))) 空气 ))) 麦克风
                                                                  │
[板上] ../build/sdr_rx  ← ALSA 采集 → 去交织 → 模型(滤波) → mat_sink 写盘
                                                                  │ single_f.mat / single_f2.mat
[PC]  scp 拉回 → load + tools/spectrum.m → FFT 看峰值(应在 1000 Hz)
```
