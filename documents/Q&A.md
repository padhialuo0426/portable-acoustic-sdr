...
- **板上编译刻意保留手动**（`make` 那一步要自己敲）——它是教学平台，理解
  "源码在板上用板子自己的 gcc 编译"这件事本身就是目的，封成黑盒反而丢了学习价值。
- **其余步骤由图形界面** `host/gui.m`（两个实验各一个）**代劳**：点「① 自检」会
  连上板子、**把 Makefile/.c/.h 打包传过去**、**`arecord -l` 现场枚举采集设备**；
  点「③ 一键声学实测」把「板上启动采集 → 本机放音 → 取回 .mat → 解码」一次跑完。
  原因很实际：这些步骤要反复跑很多遍，手动做时还要盯着板子输出、卡着秒表切到
  MATLAB 放音，容易失手。首次学习建议仍按 [手把手教程](手把手部署运行教程.md)
  手动走一遍（第 2/3/4 节），理解清楚了再用界面提速。
