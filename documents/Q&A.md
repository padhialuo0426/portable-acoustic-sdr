...
手动跑 `dpsk_emit`/`bok_emit` 时没有这层保护，要自己确认系统输出在扬声器上。
...
1. **文件直喂**：`make AUDIO=file` 喂干净的 `dpsk_tx.raw`/`chirp_tx.raw`。BER=0 → 算法和编译没问题。
...
### Q17. QPSK 能还原图片，星座却固定偏转约 26.6°？

旧版接收机对两档速率都使用 16-QAM 鉴相参考，QPSK 可能锁到非对角线方向。四簇仍在正确象限时 BER 可以为零，但判决余量下降。接收模型现通过 `RxRate` 选择各自的鉴相参考和 AGC 目标。重新生成 C、同步编译并重新打开 GUI；手动接收时使用 `-b 1200`。旧符号文件不会因更新模型自动变为新结果，需要重新接收，或用保存的原始音频回放。
