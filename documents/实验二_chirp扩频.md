...
```mermaid
flowchart LR
    img["图像(基带)"] --> bits["比特流"] --> mod["BOK chirp 调制<br/>(1kHz 声载波)"] --> spk["扬声器(发射)"]
    spk -->|声波信道| mic["麦克风(接收天线)"]
    mic --> det["chirp 接收检测<br/>(本工程板端)"] --> dec["解码/判决/BER"] --> out["还原图像"]
```
