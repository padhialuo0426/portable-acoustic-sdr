...
[`sstv_emit.m`](../exp5_sstv/sstv_emit.m) 与实验二、三一样是顺序执行的 MATLAB 脚本。它依次设置参数、处理图片、写入 VIS 头、逐行扫描、积分频率得到连续相位音频，再保存与播放。模式表、扫描段和图片处理局部函数都在同一文件末尾；无需工程辅助函数或模型。修改顶部的 `imageFile`、`MODE`、`BINARY` 和播放开关后，直接运行 `sstv_emit`。GUI 的 MATLAB 分支也运行此脚本。

模型发送入口位于 `gui_support/sstv_emit_simulink.m`；在实验根目录执行 `addpath('gui_support')` 后可手动调用。它通过 `Simulink.SimulationInput` 提供图片与模式，调用 `sim` 收集 `sstv_transmit.slx` 的音频，每次输出 4800 点，最后一帧按有效长度去掉补零。图片准备、写盘和播放由该入口处理；VIS、扫描与调制在模型内执行。GUI 的模型分支使用自己的仿真适配函数，模型可以脱离 MATLAB 发送脚本运行。修改模式时序时，需同步核对发送脚本、解码器与模型参数表。
...
[`sstv_decode.m`](../exp5_sstv/sstv_decode.m) 是独立单文件顺序解码脚本，自带完整模式表、VIS 与行同步、像素重组和质量计算函数。它只读取接收 MAT 和可选参考 MAT，不调用发送机或工程算法库。

解码器先从鉴频结果识别有效 VIS，再按编号选择模式时序并预测第一行。每个行组在预计位置附近搜索 1200 Hz 同步脉冲的结束边沿，并检查脉冲内部的频率，避免把任意图像跳变当作同步。
