...
- **板端只输出复符号**，两档速率共用一个接收模型，由 `RxRate` 输入切换增益目标和鉴相参考；PC 按相同速率继续判决、
...
| `model_glue.c` | **唯一**耦合 Simulink 符号名的薄层：`v22_receive_U.AudioIn` 音频输入、`v22_receive_U.RxRate` 速率输入、`v22_receive_Y.symOut` 输出。 |
...
- 输入：**Inport `RxRate`**，`uint16` 标量，设为 1200 或 2400；运行时通过 `-b` 设置。
...
| AGC 与静噪 | 60 个复符号 → 60 个复符号 + 有效标志 | 按当前帧平均幅度归一化；QPSK 目标幅度为 1，16-QAM 为 2.99535；低于 0.001 门限时输出零。 |
| 载波相位恢复 | 60 个复符号 → 60 个校正符号 | 按速率选择鉴相参考的二阶环，`Kp=0.05`、`Ki=0.0002`，相位和积分量跨帧保留。 |
...
速率切换也会复位载波环。QPSK 每个象限只使用单位圆上的对角线参考点；
16-QAM 使用四个第一象限参考点。QPSK 若误用 16-QAM 参考，会出现约 ±26.6° 的错误锁定。

模型注释、信号标注和模块显示名称使用中文；内部块路径及 `AudioIn/RxRate/symOut` 标识保留英文，
便于脚本与生成代码按固定名称访问。
...
| `carrier_recovery.m` | 按速率选择鉴相参考；静噪、重新捕获或切档时复位载波环。 |
...
GUI 中直接选择图片和速率，接收速率随启动命令自动传到板上；手动运行时用 `-b 1200` 或 `-b 2400`。
信号时长包括前导、后导、位填充和滤波拖尾，
...
./build/v22_rx -b 2400 -d plughw:2,0 -t 7         # 默认图片示例；card 号与时长按实际改
```
...
更新接收代码后重新打开 GUI，并同步编译；旧版接收程序不支持 `-b`。
...
./tests/artifacts/build-file/v22_rx -b 2400 -d v22_tx.raw  # QPSK 改为 -b 1200
```
