...
flowchart TB
    ...
    bits --> check["HDLC 解帧 / FCS 检查"] --> result["有效图片 / 帧成功率"]
...
GUI 左侧上下显示发送点阵和接收点阵，右侧显示星座。每次任务先清除旧结果；m 序列定位成功但 FCS 失败时，本次受损点阵标为“实验对照还原（FCS 未通过）”，不计接收成功。未能可靠定位且没有有效帧时，接收图保持空白。默认星座显示“m序列测量窗口”的**实际接收点**，可切换到“有效图片帧”或“全部接收符号”。
