...
%   随忙闲切换 Enable 的控件登记进来。
    ...
    methods (Access = private)
        ...
        % 在 Windows 上要 Git Bash 才有）。口径就是「board/ 里的一切 + 共享底层
        % + 基带图片」：.c/.h/Makefile 是编译要的，board/py 与 baseband_images
        % 是板上那套免 MATLAB 脚本要的。host/ 下的 .slx 和 .m 只在 PC 上用，不上板。
        function ok = deployFiles(app)
            ...
            pats = { fullfile('common','include','*.h'), ...
                     ...
                     fullfile(expName,'board','*.c'), ...
                     fullfile(expName,'board','*.h'), ...
                     fullfile(expName,'board','py','*.py'), ...
                     fullfile(expName,'board','model','*_ert_rtw','*.c'), ...
                     fullfile(expName,'board','model','*_ert_rtw','*.h'), ...
                     fullfile(expName,'board','model','slprj','ert','_sharedutils','*.c'), ...
                     fullfile(expName,'board','model','slprj','ert','_sharedutils','*.h'), ...
            ...
            % 先删掉板上旧的 board/model：tar 解包只覆盖/新增、不删除，若这次重新
            % 生成让某个 .c 改了名或消失，残留的"孤儿 .c"会被 Makefile 的 *.c 通配
            % 编进去而报错（教程 6.1 提醒过的坑）。生成代码独占这一个目录，所以
            % 可以整个删掉重来；手写的 board/*.c 不会改名，不动。
            %
            ...
            [st, out] = app.ssh(sprintf(['tar tzf %s >/dev/null && mkdir -p %s && ' ...
                                    'rm -rf %s/%s/board/model && tar xzf %s -C %s ' ...
                                    '--exclude=''._*'' && rm -f %s'], ...
