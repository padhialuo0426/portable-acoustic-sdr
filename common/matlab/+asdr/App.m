    ...
    methods (Access = private)
        ...
        function ok = deployFiles(app)
            ...
            pats = { fullfile('common','include','*.h'), ...
                     ...
                     fullfile(expName,'board','*_ert_rtw','*.c'), ...
                     fullfile(expName,'board','*_ert_rtw','*.h'), ...
                     fullfile(expName,'board','slprj','ert','_sharedutils','*.c'), ...
                     fullfile(expName,'board','slprj','ert','_sharedutils','*.h'), ...
            ...
            % 可以整个删掉重来（通配所以不必关心模型叫什么）；手写的 board/*.c
            % 不会改名，不动。
            %
